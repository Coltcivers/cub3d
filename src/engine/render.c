	...
	t_render renderer, t_point ray_dir)
{
	...
	i = 0;
	floor_c = ((game->display.level.textures[F].image.color[1] & 0xff) << 16)
		+ ((game->display.level.textures[F].image.color[2] & 0xff) << 8)
		+ (game->display.level.textures[F].image.color[3] & 0xff);
	ceiling_c = ((game->display.level.textures[C].image.color[1] & 0xff) << 16)
	...
	while (i < renderer.draw_start)
		pixel_put(&game->display.win.buffer, renderer.ray, i++, floor_c);
	if (dda->side == 0)
	...
	while (i < game->display.win.size[HEIGHT])
		pixel_put(&game->display.win.buffer, renderer.ray, i++, ceiling_c);
}
